...
import styles from '@/styles/cuidadopersonal.module.css';

/**
 * Productos de Cuidado Personal - IZA & CAS
 * 
 * Categoría dedicada a productos de cuidado personal
...
export default function CuidadoPersonalPage() {
  ...
  const { products: cuidadopersonalProducts, loading, error } = useProducts('cuidadopersonal');
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [showFilters, setShowFilters] = useState(false);

  // Aplicar filtros a los productos
  const filteredProducts = applyFilters(cuidadopersonalProducts);
  ...
  return (
    <>
        ...
        <div className="container">
          ...
          <div style={{ textAlign: 'center', marginBottom: '40px', paddingTop: '20px' }}>
            ...
            <p style={{ 
              ...
              Productos para tu cuidado personal y bienestar
            </p>
            ...
            {!loading && !error && (
              <div style={{
                ...
                <span style={{
                  ...
                  {filteredProducts.length === cuidadopersonalProducts.length ? (
                    `${cuidadopersonalProducts.length} producto${cuidadopersonalProducts.length !== 1 ? 's' : ''}`
                  ) : (
                    <>
                      {cuidadopersonalProducts.length} producto{cuidadopersonalProducts.length !== 1 ? 's' : ''} disponible{cuidadopersonalProducts.length !== 1 ? 's' : ''}
      ...
      <FilterPanel
        ...
        products={cuidadopersonalProducts}
      />
