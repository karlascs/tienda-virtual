 ...
 * 
 * Categoría dedicada a tecnología y accesorios
...
export default function TecnologiaPage() {
  ...
  const { products: tecnologiaProducts, loading, error } = useProducts('tecnologia');
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [showFilters, setShowFilters] = useState(false);

  // Aplicar filtros a los productos
  const filteredProducts = applyFilters(tecnologiaProducts);
        ...
        <div className="container">
          ...
          <div style={{ textAlign: 'center', marginBottom: '40px', paddingTop: '20px' }}>
            ...
            <p style={{ 
              ...
              Dispositivos tecnológicos y accesorios de última generación
            </p>
            ...
            {!loading && !error && (
              <div style={{
                ...
                <span style={{
                  ...
                  {filteredProducts.length === tecnologiaProducts.length ? (
                    `${tecnologiaProducts.length} producto${tecnologiaProducts.length !== 1 ? 's' : ''}`
                  ) : (
                    <>
                      {tecnologiaProducts.length} producto{tecnologiaProducts.length !== 1 ? 's' : ''} disponible{tecnologiaProducts.length !== 1 ? 's' : ''}
